...
if __name__ == '__main__':
    ...
    doc_path_list = sorted(glob(f"{pdf_files_dir}/*.pdf", recursive=True))
    print("文件数量: ", len(doc_path_list))
    # 过滤已处理的文件
    md_path_list = sorted(glob(f"{output_dir}/**/*.md", recursive=True))
    md_name_list = [os.path.basename(md_path).replace('.md', '') for md_path in md_path_list]
    doc_path_list = [file_path for file_path in doc_path_list if os.path.splitext(os.path.basename(file_path))[0] not in md_name_list]
    print("过滤后文件数量: ", len(doc_path_list))
