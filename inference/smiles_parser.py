...
def generate_response(prompt):
    ...
    try:
        response = client.chat.completions.create(
            model="ft:gpt-4o-2024-08-06:personal:gpt-4o-hydrophilic-polymer:BV6KUljU",
            messages=[
...
def main():
    ...
    # 设置要执行的任务数量
    total_tasks = 2300  # 可根据需要修改

    print("\n" + "="*60)
    print("开始多线程批量生成数据")
    print("="*60)
    print(f"任务数量：{total_tasks} 条")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        # 提交所有任务，每个任务使用相同的prompt
        futures = {
            executor.submit(process_single_task, i+1, prompt): i+1 
